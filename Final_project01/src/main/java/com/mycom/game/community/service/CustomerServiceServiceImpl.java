...
public class CustomerServiceServiceImpl implements CustomerServiceService{
	...
	public void insert(HttpServletRequest request, CustomerServiceDto dto) {
		...
		//비밀글 여부
		String secret=request.getParameter("secret2");
	...
	public void commentDelete(HttpServletRequest request) {
		 ...
		 */
		String id=(String)request.getSession().getAttribute("id");
		CustomerServiceCommentDto dto=new CustomerServiceCommentDto();
		dto.setNum(num);
		//해당글의 작성자를 읽어와서
		String writer=cscommentDao.getData(dto).getWriter();
		if(!id.equals(writer)) {
			throw new ForbiddenException();
		}
		//2. commentDao 를 이용해서 글 삭제
